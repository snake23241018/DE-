...
if ($conn->connect_error) {
  ...
  //Распаралель условие для ролей "User" и "Admin"!!!
    if (($query->num_rows > 0) && ($EMAIL == $_COOKIE['Email']) && ($PASSWORD == $_COOKIE['Password'])) {
      ...
      setcookie("Email_Cookie", $_COOKIE['Email'], time() + 3600, "/", "de.votkpromtech.ru");
      setcookie("Login_Cookie", $_COOKIE['Login'], time() + 3600, "/", "de.votkpromtech.ru");
      setcookie("Password_Cookie", $_COOKIE['Password'], time() + 3600, "/", "de.votkpromtech.ru");
      setcookie("FIO_Cookie", $_COOKIE['FIO'], time() + 3600, "/", "de.votkpromtech.ru");
      setcookie("Phone_Cookie", $_COOKIE['Phone'], time() + 3600, "/", "de.votkpromtech.ru");
      setcookie("Role_Cookie", $_COOKIE['Role'], time() + 3600, "/", "de.votkpromtech.ru");
            
      echo "<script>window.location.href = 'http://de.votkpromtech.ru/userpanel.php';</script>";
      
    } else {
      // echo "<script>alert('Неверно указан логин или пароль!'); window.location.href = 'http://de.votkpromtech.ru/auth.php';</script>";
